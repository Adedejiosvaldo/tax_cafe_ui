...
const UnorderedList = ({ className, ...props }: UnorderedListProps) => (
  <ul
    className={cn(
      ...
      'flex list-disc flex-col pl-10 text-primary'
    )}
    {...filterProps(props)}
  />
)
...
const OrderedList = ({ className, ...props }: OrderedListProps) => (
  <ol
    className={cn(
      className,
      PARAGRAPH_SIZES.body,
      'flex list-decimal flex-col pl-10 text-primary'
    )}
    {...filterProps(props)}
  />
)
...
const Paragraph = ({ className, ...props }: ParagraphProps) => (
  <div
    className={cn(className, PARAGRAPH_SIZES.body, 'text-primary')}
    {...filterProps(props)}
  />
)
...
const ItalicText = ({ className, ...props }: ItalicTextProps) => (
  <i
    className={cn(className, 'italic', PARAGRAPH_SIZES.body, 'text-primary')}
    {...filterProps(props)}
  />
)
...
const StrongText = ({ className, ...props }: StrongTextProps) => (
  <strong
    className={cn(className, 'text-sm font-semibold text-primary')}
    {...filterProps(props)}
  />
)
...
const Heading1 = ({ className, ...props }: HeadingProps) => (
  <h1 className={cn(className, HEADING_SIZES[3], 'text-primary')} {...filterProps(props)} />
)

const Heading2 = ({ className, ...props }: HeadingProps) => (
  <h2 className={cn(className, HEADING_SIZES[3], 'text-primary')} {...filterProps(props)} />
)

const Heading3 = ({ className, ...props }: HeadingProps) => (
  <h3 className={cn(className, PARAGRAPH_SIZES.lead, 'text-primary')} {...filterProps(props)} />
)

const Heading4 = ({ className, ...props }: HeadingProps) => (
  <h4 className={cn(className, PARAGRAPH_SIZES.lead, 'text-primary')} {...filterProps(props)} />
)
    ...
    <div className="w-full max-w-xl">
      {error ? (
        <div className="bg-secondary/50 text-muted flex h-40 flex-col items-center justify-center gap-2 rounded-md">
          ...
          >
            {typeof src === 'string' ? src : 'Invalid URL'}
          </Link>
        </div>
      ) : (
        <Image
